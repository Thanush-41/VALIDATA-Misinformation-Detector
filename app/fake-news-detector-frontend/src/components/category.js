    ...
    Axios.get('http://127.0.0.1:8000/api/category/' + categoryParam + '/')
      .then((response) => {
        ...
        if (response.data.length === 0) {
          ...
          toast.error("No news data available for this category. Please try again later or select another category.");
      ...
      { newsData.length > 0 ? (
        ...
        <Container className='new-news-container'>
          <div style={{textAlign: 'center', padding: '50px'}}>
            ...
            <p>Please select another category from the menu above.</p>
          </div>
        </Container>
      }
