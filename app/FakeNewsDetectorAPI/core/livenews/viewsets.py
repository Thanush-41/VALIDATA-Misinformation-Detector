...
def get_new_news_from_api_and_update():
    ...
    # Fetch more results and show all pillar names
    news_data = requests.get("https://content.guardianapis.com/search?api-key=e705adff-ca49-414e-89e2-7edede919e2e&show-fields=all&page-size=50")
    news_data = news_data.json()

    # Check if the API response is valid
    if "response" not in news_data or "results" not in news_data.get("response", {}):
        print(f"Invalid API response: {news_data}")
        return
...
class LiveNewsByCategory(viewsets.ViewSet):
    def list(self, request, category=None):
        if category is not None:
            # Try to match both pillarName (news_category) and sectionId
            # First try exact match with pillarName
            live_news = LiveNews.objects.filter(news_category__iexact=category).order_by('-id')[:10]
            
            # If no results, try matching with section_id
            if not live_news.exists():
                live_news = LiveNews.objects.filter(section_id__iexact=category).order_by('-id')[:10]
            
            # If still no results, try matching with section_name
            if not live_news.exists():
                live_news = LiveNews.objects.filter(section_name__iexact=category).order_by('-id')[:10]
